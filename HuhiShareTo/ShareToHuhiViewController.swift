...
class ShareToHuhiViewController: SLComposeServiceViewController {
    ...
    private func urlScheme(for url: String) -> URL? {
        return URL(string: "huhi://open-url?url=\(url)")
    }
    ...
    override func configurationItems() -> [Any]! {
        ...
        provider.loadItem(of: provider.isUrl ? kUTTypeURL : kUTTypeText) { item, error in
            ...
            if let huhiUrl = urlString.addingPercentEncoding(withAllowedCharacters: .alphanumerics).flatMap(self.urlScheme) {
                self.handleUrl(huhiUrl)
            }
        }
