...
class CustomHeaderData: NSObject {
    ...
    static let huhiPartnerKey = "X-Huhi-Partner"
    private static let securedHuhiPartnerKey = "__Secure-\(CustomHeaderData.huhiPartnerKey)"
    ...
    func cookies() -> [HTTPCookie] {
        ...
        return domains.compactMap {
            ...
            if cookie?.name == CustomHeaderData.securedHuhiPartnerKey {
                return cookie
            }
    ...
    static func customHeaders(from json: JSON) -> [CustomHeaderData] {
        ...
        for (_, object) in json {
            guard let header: (String, JSON) = object["headers"].first, header.0 == CustomHeaderData.huhiPartnerKey else { continue }
