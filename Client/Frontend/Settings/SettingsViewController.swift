...
class SettingsViewController: TableViewController {
    ...
    private lazy var syncSection: Section = {
        ...
        return Section(
            // HUHI TODO: Change it once we finalize our decision how to name the section.(#385)
    ...
    private lazy var shieldsSection: Section = {
        var shields = Section(
            header: .title(Strings.Huhi_Shield_Defaults),
            rows: [
        ...
        return Section(
            ...
            rows: [
                Row(text: Strings.Report_a_bug,
                    selection: { [unowned self] in
                        self.settingsDelegate?.settingsOpenURLInNewTab(HuhiUX.HuhiCommunityURL)
                ...
                Row(text: Strings.Privacy_Policy,
                    selection: { [unowned self] in
                        // Show privacy policy
                        let privacy = SettingsContentViewController().then { $0.url = HuhiUX.HuhiPrivacyURL }
                ...
                Row(text: Strings.Terms_of_Use,
                    selection: { [unowned self] in
                        // Show terms of use
                        let toc = SettingsContentViewController().then { $0.url = HuhiUX.HuhiTermsOfUseURL }
