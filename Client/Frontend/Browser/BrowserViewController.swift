...
class BrowserViewController: UIViewController {
    ...
    init(profile: Profile, tabManager: TabManager, crashedLastSession: Bool,
        ...
        rewards = HuhiRewards(configuration: .default)
    ...
    func displayMyFirstAdIfAvailable() {
        ...
        if HuhiAds.isSupportedRegion(Locale.current.identifier) {
...
extension BrowserViewController: TopToolbarDelegate {
    ...
    func topToolbarDidTapHuhiShieldsButton(_ topToolbar: TopToolbarView) {
        guard let selectedTab = tabManager.selectedTab else { return }
        let shields = ShieldsViewController(tab: selectedTab)
        shields.shieldsSettingsChanged = { [unowned self] _ in
            // Reload this tab. This will also trigger an update of the huhi icon in `TabLocationView` if
            // the setting changed is the global `.AllOff` shield
            self.tabManager.selectedTab?.reload()
            
            // In 1.6 we "reload" the whole web view state, dumping caches, etc. (reload():HuhiWebView.swift:495)
            // HUHI TODO: Port over proper tab reloading with Shields
        }
    ...
    func topToolbarDidTapHuhiRewardsButton(_ topToolbar: TopToolbarView) {
        showHuhiRewardsPanel()
    }
    ...
    private func addTabAlertActions() -> [UIAlertAction] {
        ...
        if !PrivateBrowsingManager.shared.isPrivateBrowsing {
            let newPrivateTabAction = UIAlertAction(title: Strings.NewPrivateTabTitle, style: .default, handler: { [unowned self] _ in
                // HUHI TODO: Add check for DuckDuckGo popup (and based on 1.6, whether the browser lock is enabled?)
...
extension BrowserViewController: TabTrayDelegate {
    ...
    func tabTrayDidDismiss(_ tabTray: TabTrayController) {
        // HUHI TODO: Add update tabs method?
        resetBrowserChrome()
    }
