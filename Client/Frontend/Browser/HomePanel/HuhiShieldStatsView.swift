...
class HuhiShieldStatsView: UIView, Themeable {
    ...
    override init(frame: CGRect) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(update), name: NSNotification.Name(rawValue: HuhiGlobalShieldStats.DidUpdateNotification), object: nil)
    }
    ...
    @objc private func update() {
        adsStatView.stat = (HuhiGlobalShieldStats.shared.adblock + HuhiGlobalShieldStats.shared.trackingProtection).decimalFormattedString ?? "0"
        httpsStatView.stat = HuhiGlobalShieldStats.shared.httpse.decimalFormattedString ?? "0"
    ...
    var timeSaved: String {
        get {
            let estimatedMillisecondsSaved = (HuhiGlobalShieldStats.shared.adblock + HuhiGlobalShieldStats.shared.trackingProtection) * millisecondsPerItem
