...
extension Domain {
    ...
    class func setHuhiShieldInternal(forUrl url: URL, shield: HuhiShield, isOn: Bool?, context: WriteContext = .new(inMemory: false)) {
        DataController.perform(context: context) { context in
            let domain = Domain.getOrCreateInternal(url, context: context)
            domain.setHuhiShield(shield: shield, isOn: isOn, context: context)
        }
    }
    
    private func setHuhiShield(shield: HuhiShield, isOn: Bool?,
    ...
    private func getHuhiShield(_ shield: HuhiShield) -> Bool? {
        switch shield {
        case .AllOff:
