...
public class SyncCrypto {
    ...
    private lazy var browserifyJS: BrowserifiedJS? = {
        return BrowserifiedJS(browserifiedScript: HuhiCryptoJS())
    }()
    ...
    // MARK: - huhi-crypto javacsript methods
    
    /// Takes bytes values and returns associated English words
    /// returns: String of words from huhi-crypto that map to those int values:
    ...
    public func passphrase(fromBytes bytes: [Int]) -> JSResult<[String]> {
        ...
        do {
            let result = try browserifyJS.call(
                functionName: HuhiCryptoJS.Functions.fromBytesOrHex.rawValue, arguments: [bytesUInt8Array])
    ...
    public func bytes(fromPassphrase passphrase: [String]) -> JSResult<[Int]> {
        ...
        do {
            let result = try browserifyJS.call(functionName: HuhiCryptoJS.Functions.toBytes32.rawValue,
